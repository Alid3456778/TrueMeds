...
app.get("/products", async (req, res) => {
  ...
  try {
    ...
    if (categoryID === "all") {
      ...
      query = "SELECT category_id,image_url,product_name,trade_names,product_id,stocks FROM products ORDER BY product_name";
    } else { 
      query =
        `SELECT
...
// app.post("/api/voice-order-test", async (req, res) => {
//   try {
//     const data = req.body;

//     console.log("Voice Order Received:", data);

//     await pool.query(
//       "INSERT INTO voice_orders_test (order_data) VALUES ($1)",
//       [data]
//     );

//     res.status(200).json({ message: "Voice order saved successfully" });
//   } catch (error) {
//     console.error("Error saving voice order:", error);
//     res.status(500).json({ error: "Failed to save voice order" });
//   }
// });
app.post("/api/voice-full-order-test", async (req, res) => {
  try {
    const data = req.body;

    console.log("Full Voice Order Received:", data);
    ...
    res.status(200).json({ message: "Full voice order saved successfully" });

  } catch (error) {
    console.error("Error saving full voice order:", error);
    res.status(500).json({ error: "Failed to save full voice order" });
  }
});
